...
const AddSlotDialog: React.FC<AddSlotDialogProps> = ({
  ...
  return (
    <div className="fixed inset-0 z-[100] flex items-center justify-center" role="dialog" aria-modal="true">
      ...
      <div className="relative z-10 w-full max-w-5xl max-h-[90vh] overflow-hidden rounded-xl border border-border-primary bg-bg-card shadow-2xl mx-4">
        <div className="flex items-start justify-between gap-3 border-b border-border-subtle px-5 py-4">
          ...
          <button onClick={onClose} className="p-1.5 rounded-lg text-text-tertiary hover:text-text-primary hover:bg-bg-tertiary">
            <X className="w-4 h-4" />
          </button>
        </div>

        <div className="overflow-y-auto max-h-[calc(90vh-145px)] px-5 py-4">
          <SlotConfigForm
            mode="add"
            draft={draft}
            onChange={setDraft}
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 border-t border-border-subtle px-5 py-4">
          ...
          <div className="flex gap-2 justify-end">
            <button onClick={onClose} className="px-4 py-2 rounded-lg text-sm font-medium border border-border-primary text-text-secondary hover:bg-bg-tertiary transition-colors">
              Close
            </button>
            <button
              disabled={Boolean(validationError) || preflightLoading}
              onClick={() => onPreflight(draft.slot_id, buildSlotPreflightRequest(draft))}
              className="inline-flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium border border-accent-primary/30 text-accent-primary bg-accent-primary/10 hover:bg-accent-primary/20 disabled:opacity-50 disabled:cursor-not-allowed transition-colors"
            >
