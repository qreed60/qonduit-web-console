...
import { RagRegistryProject, RagUpdateProjectRequest } from '../types';
import { updateRegistryProject } from '../services/ragProjectsApi';
import Toast from './Toast';

interface EditProjectDialogProps {
  open: boolean;
  project: RagRegistryProject | null;
  onClose: () => void;
...
const EditProjectDialog: React.FC<EditProjectDialogProps> = ({ open, project, onClose, onSuccess }) => {
  ...
  const handleSave = async () => {
    ...
    setSaving(true);
    try {
      const req: RagUpdateProjectRequest = {
        ...
        <div className="relative bg-bg-card border border-border-primary rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-lg max-h-[90vh] sm:max-h-[85vh] flex flex-col animate-slide-up sm:animate-slide-in-down z-10">
          ...
          <div className="flex-1 overflow-y-auto px-4 sm:px-6 py-4 space-y-4">
            ...
            <div>
              ...
              />
              <p className="text-[10px] text-text-tertiary mt-1">Project ID cannot be changed.</p>
            </div>
            ...
            <div>
              ...
              <input
                type="text"
                value={project.qdrant_collection}
                disabled
                className="w-full px-4 py-3 bg-bg-secondary/50 border border-border-subtle rounded-xl text-text-tertiary font-mono min-h-[48px]"
              />
            </div>
            ...
            <div className="p-3 bg-bg-secondary rounded-lg border border-border-subtle space-y-1">
              <p className="text-[10px] text-text-tertiary">
                Collections: <span className="text-text-secondary">{project.collections_count}</span>
              </p>
              <p className="text-[10px] text-text-tertiary">
                Points: <span className="text-text-secondary">{project.points_count.toLocaleString()}</span>
              </p>
              {project.created_at && (
                <p className="text-[10px] text-text-tertiary">
                  Created: <span className="text-text-secondary">{new Date(project.created_at).toLocaleString()}</span>
                </p>
              )}
            </div>
          </div>
