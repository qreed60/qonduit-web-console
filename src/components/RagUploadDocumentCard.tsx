     ...
     <div className="bg-bg-card rounded-xl border border-border-primary p-4 sm:p-5 shadow-card">
       ...
       <div className="space-y-3">
         ...
         {availableCollections.length > 1 && (
           <div>
             <label className="block text-xs text-text-tertiary mb-1 font-medium">
               Collection (optional)
             </label>
             <select
               value={selectedCollection || ''}
               onChange={(e) => setSelectedCollection(e.target.value || undefined)}
               className="w-full px-3 py-2.5 bg-bg-secondary border border-border-primary rounded-lg text-sm text-text-primary focus:outline-none focus:border-accent-primary/50 transition-colors min-h-[44px]"
             >
         ...
         <div>
           ...
           >
             ...
             {selectedFile ? (
               ...
               <p className="text-sm text-text-tertiary">Click to select a file</p>
             )}
           </div>
         </div>
         ...
         <div>
           <p className="text-xs text-text-tertiary mb-1">Supported:</p>
           <div className="flex gap-1 overflow-x-auto -mx-1 px-1 pb-1">
             {SUPPORTED_FILE_HINTS.map(f => (
               <span
                 key={f}
                 className="px-1.5 py-0.5 rounded text-[10px] bg-bg-tertiary text-text-tertiary font-mono flex-shrink-0"
               >
                 {f}
               </span>
             ))}
           </div>
           <p className="text-xs text-text-tertiary mt-1">{GOOGLE_EXPORT_HELP}</p>
         </div>
